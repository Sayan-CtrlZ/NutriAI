                            ...
                            <div className="flex items-center gap-3">
                                    ...
                                    <div className="shrink-0 relative w-14 h-14 flex items-center justify-center">
                                        <svg className="transform -rotate-90 w-14 h-14">
                                            ...
                                            <circle cx="28" cy="28" r="24" stroke="currentColor" strokeWidth="5" fill="transparent"
                                                ...
                                                strokeLinecap="round"
                                                className={`${analysisResult.health_score >= 70 ? "text-emerald-500" :
                                ...
                                <div>
                                    ...
                                    <h3 className="font-bold text-white text-lg leading-tight break-words pl-2">
                                        {analysisResult?.health_score >= 70 ? "Excellent Choice" : analysisResult?.health_score >= 40 ? "Moderate" : "Changes Recommended"}
                                    </h3>
                                </div>
                            </div>
                            <div className="text-xs text-slate-300 font-light leading-relaxed break-words px-1 pl-4">
                                    ...
                                    <div key={idx} className="bg-white/5 py-3 pr-3 pl-5 mx-2 rounded-lg border border-white/10 flex gap-3 items-start hover:bg-white/10 transition-colors">
                                        ...
                                        <div className="flex-1">
                                            <div className="flex justify-between items-baseline">
                                                <h4 className={`text-base font-bold ${textColor}`}>{point.ingredient}</h4>
                                            </div>
                                            <p className="text-xs text-slate-300 font-light leading-relaxed mt-1 opacity-90 break-words pl-1">{point.why_it_matters}</p>
                                        </div>
                                    </div>
                                )
                            })}
            ...
            <div className="w-full md:w-[38%] flex flex-col bg-slate-900/40 backdrop-blur-md rounded-3xl md:rounded-3xl relative overflow-hidden flex-1 min-h-[450px] border border-white/5">
                ...
                <div className="p-5 border-b border-slate-500/20 bg-slate-900/30 flex items-center gap-3">
                    ...
                    <div>
                        ...
                        <p className="text-slate-400 text-xs">Ask anything about your analysis</p>
                    </div>
                </div>
                ...
                <div className="p-4 md:p-5 border-t border-white/10 bg-slate-900/30">
                    <div className="flex gap-3 relative">
                        <input
                            ...
                            className="w-full bg-white/5 border border-white/10 rounded-2xl pl-5 pr-14 py-4 text-white placeholder-slate-500/50 focus:outline-none focus:ring-1 focus:ring-indigo-400 focus:bg-white/10 transition-all font-light"
                        />
                        <button
                            onClick={handleSend}
                            disabled={!input.trim() || isChatting}
                            className="absolute right-2 top-2 bottom-2 aspect-square bg-gradient-to-tr from-violet-600 to-indigo-600 hover:from-violet-500 hover:to-indigo-500 disabled:opacity-0 disabled:scale-90 text-white rounded-xl transition-all shadow-lg shadow-indigo-900/20 flex items-center justify-center group"
                        >
